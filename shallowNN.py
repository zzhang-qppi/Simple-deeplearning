...
def initialize_parameters(n_dimensions_f):
	n_hidden, n_A0, n_A_output = n_dimensions_f
	W1 = np.random.randn(n_hidden, n_A0) * 0.01
	b1 = np.zeros(n_hidden, 1)
	W2 = np.random.randn(n_A_output, n_hidden)
	b2 = np.zeros(n_A_output, 1)
...
def two_layers_learning(train_X_f, train_Y_f, n_dimensions_f, l_rate_f, n_iterations_f):
